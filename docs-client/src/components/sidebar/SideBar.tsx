...
export default function Sidebar({ open, setOpen }: any) {
  ...
  return (
    <div>
      ...
      {/* Sidebar */}
      <div
        className={`fixed top-0 left-0 z-[100] h-full w-64 min-w-64 transform bg-gradient-to-b from-white via-gray-50/50 to-white border-r border-gray-200/60 shadow-2xl transition-all duration-300 ease-in-out lg:z-[100] lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"
          }`}
      >
        {/* Logo Section */}
            ...
            {routes.map((route, index) => {
              ...
              return (
                <Link
                  key={route.href}
                  href={route.href}
                  className={`group relative flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium transition-all duration-200 ease-in-out ${isActive
                      ? "bg-gradient-to-r from-blue-500 to-indigo-600 text-white shadow-lg shadow-blue-500/30 transform"
                      : "hover:bg-gradient-to-r hover:from-gray-50 hover:to-blue-50 hover:text-blue-700 text-gray-700 hover:transform"
                    }`}
                >
